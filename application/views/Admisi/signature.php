...
<body>
  <?php echo form_open(base_url()."Admisi/buat_signature",array("id"=>"form_signature"))?>
<div>
<div id="content">
	<div id="signatureparent">
		<!-- <div>Silahkan Tanda Tangan Di Bawah</div> -->
		<div id="signature"></div></div>
    <!-- <input type="number" name="nokun" id="nokun" placeholder="no kunjungan" value=""> -->
    <select id="nokun" name="nokun">
      <?php foreach ($kunjungan as $value): ?>
        <option value="<?php echo $value->no_urutkunjungan?>"><?php echo $value->noRM."  |  ".$value->namapasien?></option>
      <?php endforeach; ?>
    </select>
	<div id="tools"></div>
...
<?php if ($this->session->flashdata()){if ($this->session->flashdata('pesan')==1){
?>
  <script>alert("Berhasil membuat signature")</script>
<?php
}else{?>
    <script>
      alert("No kunjungan tidak ditemukan");
      // var $sigdiv = $("#signature").jSignature("importData",)
    </script>
<?php
}
}
...
$(document).ready(function() {
	...
	var $sigdiv = $("#signature").jSignature({height:500,width:1300})
