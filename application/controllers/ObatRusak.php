<?php
  ...
  class ObatRusak extends CI_Controller{
    ...
    function insert(){
      ...
      for($i=0;$i<$count;$i++){
        ...
        $this->db->insert('obat_rusak', $detail_rusak);
        $update = array(
          'stok' => $stok,
        ...
        if ($detail['hangus']==null) {
          $jml = $jml_satuan_kecil ;
        }else{
          $jml = $detail['hangus']+$jml_satuan_kecil;
        }
        $this->db->where('iddetail_pembelian_obat',$idp[$i]);
        $this->db->update('detail_pembelian_obat',array('hangus'=>$jml));
      ...
      $jurnal = array(
        'tanggal' => date("Y-m-d"),
        'keterangan' => 'Obat rusak pada tanggal '.date("d-m-Y"),
        'norek' => '60008',
        'debet' => $kerugian,
        'kredit' => 0,
